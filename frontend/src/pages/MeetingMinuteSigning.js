...
export default function MeetingMinuteSigning() {
  ...
  useEffect(() => {
    meetingMinuteService.publicMinute(token).then((response) => setMinute(response.data)).catch((error) => setMessage({ severity: 'error', text: error.response?.data?.message || 'El enlace no es válido o venció.' })).finally(() => setLoading(false));
  }, [token]);
  useEffect(() => {
  ...
  const sign = async () => {
    setWorking(true); setMessage(null);
    try { await meetingMinuteService.sign(token, { participant_id: participantId, otp, privacy_accepted: privacyAccepted, signature_data: canvasRef.current.toDataURL('image/png') }); setSigned(true); setMessage({ severity: 'success', text: 'Firma guardada y vinculada al acta.' }); }
  ...
  return <Box sx={{ minHeight: '100vh', bgcolor: '#f4f7fb', p: { xs: 1.5, sm: 3, md: 5 } }}><Card sx={{ maxWidth: 760, mx: 'auto', borderRadius: 4, boxShadow: '0 18px 50px rgba(23,59,115,.15)' }}><Box sx={{ p: { xs: 2.5, md: 4 }, background: 'linear-gradient(135deg,#214c9c,#315ee8)', color: '#fff' }}><Typography fontWeight={900} fontSize={13}>SIAC · UNIVERSIDAD CESMAG</Typography><Typography variant="h4" fontWeight={950}>Firma electrónica del acta</Typography><Typography sx={{ opacity: .9 }}>{minute?.code} · {minute?.content?.objetivo?.[0]}</Typography></Box><CardContent sx={{ p: { xs: 2.5, md: 4 } }}>
    <Alert severity="info" sx={{ mb: 2.5 }}>Puede firmar desde celular, tableta o computador. La firma queda vinculada a esta versión del acta.</Alert>{message && <Alert severity={message.severity} sx={{ mb: 2.5 }}>{message.text}</Alert>}
