...
import {
  Add, Close, ContentCopy, DeleteOutline, Download, PersonSearch,
...
export default function MeetingMinuteFormDialog({ open, document, user, onClose }) {
  ...
  const publish = async () => {
    ...
    try {
      ...
      enqueueSnackbar('Firmas por QR habilitadas.', { variant: 'success' });
  ...
  return <>
      ...
      <DialogContent sx={{ p: { xs: 1.5, md: 3 } }}>
        <Stack direction={{ xs: 'column', lg: 'row' }} gap={2.5} alignItems="flex-start">
          <Stack gap={2} sx={{ width: { xs: '100%', lg: '46%' } }}>
            ...
            {locked && !allSigned && <Alert severity="info">El acta está habilitada para firmas. Use “Actualizar firmas” para consultar quiénes ya firmaron.</Alert>}
    ...
    <Dialog open={Boolean(qr)} onClose={() => setQr(null)} maxWidth="xs" fullWidth><DialogTitle fontWeight={900}>Firmas habilitadas</DialogTitle><DialogContent><Stack alignItems="center" gap={1.5}><Alert severity="success">Los participantes pueden escanear este QR desde cualquier dispositivo.</Alert>{qr?.qr_data_url && <Box component="img" src={qr.qr_data_url} alt="QR para firmar" sx={{ width: 260, height: 260 }} />}<TextField fullWidth size="small" value={qr?.signing_url || ''} InputProps={{ readOnly: true }} /><Button startIcon={<ContentCopy />} onClick={() => { navigator.clipboard.writeText(qr?.signing_url || ''); enqueueSnackbar('Enlace copiado.', { variant: 'success' }); }}>Copiar enlace</Button></Stack></DialogContent><DialogActions><Button onClick={() => setQr(null)}>Cerrar</Button></DialogActions></Dialog>
  </>;
}
