...
function ReporteSalidaSeguimiento({ onBack }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const response = await reporteSalidaService.listarSolicitudes({ page: 1, limit: 50, estado });
  ...
  const summary = useMemo(() => {
    ...
    return { total, pendientes, personales, finalizadas };
  }, [rows]);
      ...
      <Box>
        ...
        <Paper elevation={0} sx={{ border: '1px solid #dbe6f5', borderRadius: 4, bgcolor: '#fff', mb: 2.2, overflow: 'hidden' }}>
          <Box sx={{ p: { xs: 2, md: 2.6 }, borderBottom: '1px solid #e2e8f0' }}>
            ...
            <Typography sx={{ color: '#475569', mt: 0.5 }}>Centro de control para solicitudes, aprobaciones y tiempos gestionados por Gestion Humana.</Typography>
          </Box>
          ...
          <Box sx={{ p: { xs: 2, md: 2.4 } }}>
            <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5} justifyContent="space-between" alignItems={{ xs: 'stretch', md: 'flex-start' }}>
              <Box>
                <Typography sx={{ fontWeight: 950, fontSize: 24, color: '#0f172a' }}>Reporte de salida</Typography>
                <Typography sx={{ color: '#64748b' }}>Seguimiento de aprobaciones, notificaciones y reposicion de tiempo.</Typography>
              </Box>
            ...
            <Paper elevation={0} sx={{ border: '1px solid #dbe6f5', borderRadius: 3, overflow: 'hidden', bgcolor: '#fff' }}>
              <TableContainer>
                <Table size="small">
                  <TableHead>
                    <TableRow>
                      {['Solicitud', 'Colaborador', 'Jefe inmediato', 'Radicacion', 'Estado', 'Tiempo jefe', 'Tiempo GH', 'Reposicion'].map((label) => (
                  ...
                  <TableBody>
                    {loading ? (
                      ...
                      <TableRow><TableCell colSpan={8} sx={{ py: 3 }}><Alert severity="info">No hay solicitudes para el filtro seleccionado.</Alert></TableCell></TableRow>
                    ) : rows.map((row) => {
